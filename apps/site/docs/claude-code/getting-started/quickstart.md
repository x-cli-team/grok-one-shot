...
> **⚠️ Parity Gap:** Grok One-Shot does not yet have native installers like Claude Code's `brew install` or `curl | bash` scripts. Installation currently requires npm or Bun.
...
> **⚠️ Parity Gap:** Grok One-Shot does not have OAuth-based login like Claude Code. You must manually configure your API key via environment variable.
...
> **⚠️ Parity Gap:** Grok One-Shot does not have `/help` or `/resume` commands like Claude Code. Session management features are limited. Use `grok --help` for CLI options.
...
> **⚠️ Parity Gap:** Grok One-Shot does not yet support `--continue`, `--resume`, or `commit` subcommands like Claude Code. Session management features are limited.
