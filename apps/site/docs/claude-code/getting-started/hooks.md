...
> **⚠️ PARITY GAP**: Grok One-Shot does not currently implement the hooks system described in this document. This is a comprehensive Claude Code feature planned for future implementation.
...
- Manual workflow documentation in `.agent/docs/`
...
<Tip>
  For a quickstart guide with examples, see [Get started with Grok One-Shot hooks](/en/hooks-guide).
</Tip>
...
1. Send the hook input and your prompt to a fast LLM (Grok-beta)
2. The LLM responds with structured JSON containing a decision
3. Grok One-Shot processes the decision automatically
...
Runs when the user submits a prompt, before Grok processes it. This allows you
to add additional context based on the prompt/conversation, validate prompts, or
block certain types of prompts.

### Stop

Runs when the main Grok One-Shot agent has finished responding. Does not run if
...
Runs when Grok One-Shot starts a new session or resumes an existing session (which
...
Runs when a Grok One-Shot session ends. Useful for cleanup tasks, logging session
...
There are two ways for hooks to return output back to Grok One-Shot. The output
communicates whether to block and any feedback that should be shown to Grok
and the user.
...
Grok One-Shot hooks would work seamlessly with
...
Direct edits to hooks in settings files don't take effect immediately. Grok
One-Shot:
