...
class GenerativeWriteTool extends TextEditorTool {
  async generateAndCreate(
    ...
    const optimalPath =
      suggestedPath || this.suggestPath(generated.type, intent);

    // 3. Preview as Claude-style (color + explanation)
    const preview = this.formatGenerativePreview(
      generated.content,
      optimalPath,
  ...
  private formatGenerativePreview(
    content: string,
    path: string,
    explanation: string,
  ): string {
  ...
  private buildRipgrepArgs(options: GrepOptions): string[] {
    ...
    if (options.context) {
      ...
      if (options.contextBefore)
        args.push("-B", options.contextBefore.toString());
      if (options.contextAfter)
        args.push("-A", options.contextAfter.toString());
    }
...
class EnhancedTodoTool extends TodoTool {
  ...
  async updateTodoList(updates: any[]): Promise<ToolResult> {
    ...
    const currentInProgress = this.todos.filter(
      (t) => t.status === "in_progress",
    ).length;
    const newInProgress = updates.filter(
    ...
    if (currentInProgress + newInProgress > 1) {
      return {
        success: false,
        error:
          "Only ONE task can be in_progress at a time. Complete current task before starting another.",
      };
    }
...
_This comprehensive tool analysis serves as the foundation for achieving Claude Code parity and establishing Grok CLI as a competitive AI coding assistant._
