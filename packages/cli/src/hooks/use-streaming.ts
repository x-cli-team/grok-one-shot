...
// Prevent memory spikes from extremely long responses
          ...
          for await (const chunk of agent.processUserMessageStream(initialMessage)) {
            switch (chunk.type) {
              case "content":
                if (chunk.content) {
                  // Prevent memory spikes from extremely long responses
                  if (accumulatedContent.length + chunk.content.length > MAX_CONTENT_LENGTH) {
                    console.warn(`⚠️ Response content exceeded ${MAX_CONTENT_LENGTH / 1024 / 1024}MB limit, truncating`);
                    accumulatedContent = accumulatedContent.substring(0, MAX_CONTENT_LENGTH - 1000) +
                      '\n\n[Content truncated due to length limit]';
                    // Skip adding more content
                    break;
                  }
